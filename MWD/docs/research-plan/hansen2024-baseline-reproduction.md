...
本地评估同时记录 ECE（Expected Calibration Error）。在预注册 seed=42 的 benchmark 复跑中，整体测试集 ECE 为 `0.0775`；该指标用于后续质量—不确定性双门控，不参与当前模型选择分数。
...
1. 固定本次基线，不调整论文参数；
2. 实现按 `Tunnel` 的留一验证，作为工程泛化实验；
3. 在完成上述两个对照后，基于当前 ECE 接口增加温度校准和质量—不确定性双门控对照。

## 过渡区分层结果

使用与首轮基线完全相同的模型和训练数据，仅在测试集上按 `transition_zone` 分层评价：

| 测试切片 | 样本数 | Accuracy | Balanced Accuracy | Macro-F1 | ROC-AUC (OvR) |
|---|---:|---:|---:|---:|---:|
| 普通区段 | 1006 | 0.8986 | 0.9122 | 0.8860 | 0.9942 |
| 过渡区段 | 218 | 0.6193 | 0.6821 | 0.6651 | 0.9521 |

过渡区的性能明显低于普通区段，支持将地质边界模糊和标签过渡作为后续研究重点。该结果只是切片评价，不代表重新训练了一个过渡区专用模型。
