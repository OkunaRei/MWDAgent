...
中文：**面向跨隧道泛化的知识约束随钻地层识别模型优化 Agent**

英文：**A Knowledge-Constrained Agent for Optimizing MWD-Based Geological Prediction under Cross-Tunnel Generalization**
...
> Agent 能否根据跨隧道验证和地质错误切片识别模型失败原因，并在知识约束下选择更有利于工程泛化的下一步优化动作。
...
- 泛化切分：公开固定 train/test、按隧道留出、按里程顺序留出。
...
1. 在相同数据、候选模型和计算预算下，知识约束 Agent 的跨隧道结果优于固定流程和普通搜索；
2. 提升主要来自对错误切片、数据质量和空间泛化的正确诊断，而不是测试集调参；
